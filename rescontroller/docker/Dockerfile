...
RUN GOOS=linux go build -buildmode=pie -ldflags '-linkmode "external" -extldflags "-static"' -o rescontroller

RUN chmod 550 $HOME/rescontroller $HOME/start.sh &&\
...
COPY --from=builder --chown=$USER_NAME:$GROUP_NAME $HOME/rescontroller $HOME/bin
