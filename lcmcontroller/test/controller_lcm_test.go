...
func testUploadConfigV2(t *testing.T, extraParams map[string]string, path string, testDb dbAdapter.Database) {
	...
	t.Run("TestConfigUploadV2", func(_ *testing.T) {
		...
		assert.Equal(t, 400, uploadControllerLcmV2.Ctx.ResponseWriter.Status, "Config upload failed")
		...
		assert.Equal(t, 400, uploadController.Ctx.ResponseWriter.Status, "Config upload is failed")
...
func testUploadConfig3(t *testing.T, extraParams map[string]string, path string, testDb dbAdapter.Database) {
	...
	t.Run("TestConfigUploadV2", func(_ *testing.T) {
		...
		defer patch4.Reset()
		uploadControllerLcmV2.UploadConfigV2()
		assert.Equal(t, 500, uploadControllerLcmV2.Ctx.ResponseWriter.Status, "Config upload failed")

		patch11 := gomonkey.ApplyMethod(reflect.TypeOf(uploadControllerLcmV2), "IsPermitted", func(_ *controllers.LcmControllerV2 , _, _ string) (string, error) {
			return  "", nil
		})
		defer patch11.Reset()
		//test case-2 for UploadConfig of lcmController
		uploadController.UploadConfig()
		assert.Equal(t, 500, uploadController.Ctx.ResponseWriter.Status, "Config upload is failed")


		//case-2
		patch8 := gomonkey.ApplyFunc(util.ValidateSrcAddress, func(id string) error {
			return nil
		})
		defer patch8.Reset()
...
func testGetInputParametersForUploadCfg(t *testing.T, extraParams map[string]string, path string,
	...
	t.Run("TestGetInputParametersForUploadCfg", func(_ *testing.T) {
		...
		defer patch2.Reset()
		patch4:= gomonkey.ApplyFunc(util.ValidateFileExtensionEmpty, func(fileName string) error {
			return nil
		})
		defer patch4.Reset()
		patch5 := gomonkey.ApplyFunc(util.ValidateIpv4Address, func(_ string) error {
...
func testSynchronizeAppPackageUpdatedRecord1(t *testing.T, extraParams map[string]string,
	...
	t.Run("TestSynchronizeAppPackageUpdatedRecord", func(t *testing.T) {
		...
		patch13 := gomonkey.ApplyFunc(util.ValidateSrcAddress, func(id string) error {
			return nil
		})
		defer patch13.Reset()
		patch14 := gomonkey.ApplyFunc(util.ValidateAccessToken, func(_ string,
...
func testInsertOrUpdateAppInfoRecord(t *testing.T, extraParams map[string]string, path string, testDb dbAdapter.Database) {
	...
	t.Run("TestInsertOrUpdateAppInfoRecord", func(_ *testing.T) {
		...
		patch7 := gomonkey.ApplyFunc(util.ValidateSrcAddress, func(id string) error {
			return nil
		})
		defer patch7.Reset()
		...
		initializeController.HandleLoggingForFailure(clientIp, "not found")
	})
}
...
func testQueryV2(t *testing.T, extraParams map[string]string, path string, testDb dbAdapter.Database) {
	...
	t.Run("TestQueryV2", func(_ *testing.T) {
		...
		queryController.QueryV2()
		queryController.QueryKPI()
		queryController.HandleKPI(clientIp, errors.New("not found"), "404")
...
func testInsertOrUpdateAppPkgHostRecord(t *testing.T, extraParams map[string]string, path string, testDb dbAdapter.Database) {
	...
	t.Run("TestInsertOrUpdateAppPkgHostRecord", func(_ *testing.T) {
		...
		result1 := instantiateController.InsertOrUpdateAppPkgHostRecord("1.1.1.1", clientIp, tenantIdentifier, packageId,
			"distributed", "MEPM")
		...
		defer patch6.Reset()
		result := instantiateController.InsertOrUpdateAppPkgHostRecord("1.1.1.1", clientIp, tenantIdentifier, packageId,
			"pending", "MEPM")
