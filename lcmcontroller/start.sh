...
if [ ! -z "$OS_PLUGIN" ]; then
  validate_host_name "$OS_PLUGIN"
  valid_os_plugin_host_name="$?"
  if [ ! "$valid_os_plugin_host_name" -eq "0" ]; then
    echo "invalid os plugin host name"
    exit 1
  fi
else
  export OS_PLUGIN=mecm-mepm-osplugin
fi

if [ ! -z "$OS_PLUGIN_PORT" ]; then
  validate_port_num "$OS_PLUGIN_PORT"
  valid_os_plugin_port="$?"
  if [ ! "$valid_os_plugin_port" -eq "0" ]; then
    echo "invalid OS plugin port number"
    exit 1
  fi
else
  export OS_PLUGIN_PORT=8234
fi
