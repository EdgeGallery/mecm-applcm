...
RUN GOOS=linux go build -buildmode=pie -ldflags '-linkmode "external" -extldflags "-static"' -o k8splugin

RUN chmod 550 $HOME/k8splugin $HOME/start.sh &&\
    chmod -R 750 $HOME/conf/ &&\
    chmod 640 $HOME/conf/config.yaml
...
RUN chmod 750 $HOME &&\
    ...
    mkdir -p -m 750 $HOME/artifacts &&\
    mkdir -p -m 750 $HOME/artifacts/packages &&\
    chown -hR $USER_NAME:$GROUP_NAME $HOME

# Copy in the application exe.
COPY --from=builder --chown=$USER_NAME:$GROUP_NAME $HOME/k8splugin $HOME/bin
